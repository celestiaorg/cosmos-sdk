...
* (x/auth/tx) [#747](https://github.com/celestiaorg/cosmos-sdk/pull/747) The tx decoder now rejects `TxRaw` bytes that repeat `body_bytes` (field 1) or `auth_info_bytes` (field 2). Only `signatures` (field 3) may appear more than once, which is what the ADR-027 check always documented but never enforced. A repeated `body_bytes` decodes differently under different protobuf implementations (gogoproto keeps the last occurrence, protobuf-go merges them), so any component that re-parses raw tx bytes could disagree with the decoder about what the transaction contains. No standard signer produces such encodings, so no legitimate transaction is affected.

...
// loadLatestVersion loads the latest committed version and discards what an
// interrupted commit left above it in the IAVL stores. Only this path may do
// so: an explicit historical load must never delete newer state.
...
// loadVersion loads every mounted store at ver with the given IAVL load options.
...
func (rs *Store) loadCommitStoreFromParams(key types.StoreKey, id types.CommitID, params storeParams, opts iavl.LoadStoreOptions) (types.CommitKVStore, error) {
	...
	case types.StoreTypeIAVL:
		store, err := iavl.LoadStoreWithOpts(db, rs.logger, key, id, params.initialVersion, rs.iavlCacheSize, rs.iavlDisableFastNode, rs.metrics, opts)
		if err != nil {
			return nil, err
		}
